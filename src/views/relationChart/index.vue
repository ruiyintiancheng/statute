  ...
  <div>
    ...
    <div class="main-chart" :style="{width: `${chart_width}px`, height: `${chart_height}px`}">
      <lawsrelation-chart v-if="active === 0" ref="lawsRelationChart"
        :width=chart_width :height=chart_height :id=id></lawsrelation-chart>
      <lifecycle v-else-if="active === 1"
        :width=chart_width :height=chart_height :id=id></lifecycle>
      <triangle-chart v-else-if="active === 2"
        :width=chart_width :height=chart_height :id=id></triangle-chart>
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    computed: {
      ...
      chart_height() {
        return document.querySelector('.app-main').offsetHeight - 40
      }
    },
    data() {
      return {
